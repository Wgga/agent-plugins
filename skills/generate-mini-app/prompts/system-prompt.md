...
如果涉及上述场景：
1. 使用 `file_read` 工具全文读取 `${skill_assets_dir}/prompts/cloud-development.md`，作为云开发规范加载到上下文
...
**重要**：输出完成总结时，必须调用 `preview` 工具打开预览 URL（即 `preview-server.js` 输出的 `[PreviewUrl]` 地址），确保用户在聊天界面中看到可点击的预览卡片。各平台对应工具：Trae → `OpenPreview`，Cursor → `Browser`，Claude Code → 显示URL或 `open` 命令。

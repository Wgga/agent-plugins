---
name: "debugger"
description: "用于调试需要收集运行时证据的复杂问题。启动调试服务器通过 HTTP 收集日志，遵循科学调试流程（假设 → 插桩 → 复现 → 分析 → 修复 → 验证）。适用于仅通过静态代码分析无法诊断的 Bug。在用户主动要求运行时调试、或经过多轮对话仍无法通过静态分析解决问题时触发。"
platforms:
  - trae
  - claude_code
  - cursor
...
This skill follows the **Agent Plugins 1.0** standard and works across multiple AI coding platforms. The Debug Server (`tools/debug-server/python/debug-server.py`) is Python-based and platform-agnostic. Tool references below use **generic names**; map them to your platform:

| Capability | Generic Name | Trae IDE | Claude Code | Cursor / VS Code Copilot |
|---|---|---|---|---|
| Code search | `code_search` | `SearchCodebase` | `Grep` / `Glob` | `Grep` / `Glob` |
...
| Interactive feedback | `interactive_feedback` | Platform-specific | `AskUserQuestion` | Platform-specific |

> **Platform Detection**: When loaded, the AI agent should auto-detect its platform and use the corresponding tool names. The Debug Server is started via `terminal` (platform-agnostic command). Log files are read via `file_read`.
...
> - Log file naming: `debug-log-<sessionId>.ndjson`
...
- `SKILL.md`: Core definition, input/output requirements, and key constraints.
- `guides/debug-md.md`: Purpose, template, and maintenance specifications for the `debug-<sessionId>.md` file.
- `guides/workflow.md`: Detailed 11-step scientific debugging workflow.
- `guides/logging.md`: Log reporting strategy, code templates, and event structure.
- `guides/debug-server.md`: Startup, API endpoints, environment file, and lifecycle constraints for the Debug Server.
- `guides/scenarios.md`: Debugging scenario guides with hypothesis templates.
- `examples/few-shots.md`: Correct vs. incorrect examples for common scenarios.
...
This Skill mandates using `debug-<sessionId>.md` to sync progress. For detailed specifications, refer to `guides/debug-md.md`.
...
1. **Prefer Interactive Question Tools**: If the platform provides interactive question tools, **always use them** to collect user feedback. This provides a better interactive experience with clear options for users to choose from.

2. **Fallback to Text-based Inquiry**: If interactive question tools are **not available**, use clear text-based questions with numbered options for the user to respond to.
...
**Interactive tool inquiry example (Preferred):**
```
Use the platform's interactive question tool with:
...
To ensure the standardization of the debugging process, refer to `examples/few-shots.md`.

---

## Logging System (Native API Reporting)

To ensure multi-end logs can be analyzed uniformly, follow `guides/logging.md` for log reporting.

---

## Debug Server (Runtime Evidence Collection Tool)

This Skill comes with a high-performance, out-of-the-box Debug Server script. For detailed instructions, refer to `guides/debug-server.md`.
...
The debugging process strictly follows an 11-step scientific workflow. For detailed steps, refer to `guides/workflow.md`.

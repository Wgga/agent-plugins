...
| Dart | `// #region debug-point <id>` | `// #endregion` | `// #region debug-point A:state` |
...
**Dart/Flutter (Read env file, with fallback):**
```dart
// #region debug-point A:state-check
import 'dart:convert';import 'dart:io';import 'package:dio/dio.dart';void dbgPost(Map<String,dynamic> e)async{try{final f=File('.dbg/login-500-error.env');if(await f.exists()){final c=await f.readAsString();final u=RegExp(r'DEBUG_SERVER_URL=(.+)').firstMatch(c)?.group(1)??'http://127.0.0.1:7777/event';final d=Dio(BaseOptions(connectTimeout:Duration(seconds:2)));await d.post(u,data:e);}}catch(_){}}
// #endregion
```
...
Upon receiving an HTTP POST, the Debug Server appends the data to `debug-log-<sessionId>.ndjson` in the output directory. The Agent should read logs from this file for analysis.

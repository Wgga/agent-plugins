...
> **⚠️ MUST use the exact script path below. Do NOT invent commands like `npx debugger` or `debug-server` — they don't exist.**
...
**❌ Wrong (do NOT use)**:
```bash
npx debugger start --session xxx        # Does not exist
debug-server --session xxx              # Does not exist
...
If Python is unavailable, refer to `tools/debug-server/TEMPLATE.md` to implement a minimal collector in your environment's language.
...
class Handler(http.server.BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path == "/logs":
            ...
            log_path = os.path.join(
                self.server.outdir,
                f"debug-log-{self.server.session_id}.ndjson"
            )
            logs = []
    ...
    def do_POST(self):
        ...
        out_path = os.path.join(self.server.outdir, f"debug-log-{session_id}.ndjson")
...
def main():
    ...
    if args.clean:
        log_file = os.path.join(args.outdir, f"debug-log-{args.session}.ndjson")

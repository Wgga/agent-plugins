...
## Warning
**Before attempting to implement this template, ensure you have confirmed that the built-in Python script cannot run in the current environment.**

## Goal
When the built-in Python script is unavailable, quickly implement a minimal HTTP log collector based on the current environment (e.g., Go, Rust, Ruby, Node.js, etc.).
...
4. Appends events to `<outdir>/debug-log-<sessionId>.ndjson` file
...
def handle_post(request):
    ...
    filepath = f"{outdir}/debug-log-{session_id}.ndjson"
...
## Startup Output
Must output the following structured marker for the agent to detect configuration:
```
@@DEBUG_SERVER_INFO
{
  ...
  "log_file": "<absolute_outdir>/debug-log-<sessionId>.ndjson",

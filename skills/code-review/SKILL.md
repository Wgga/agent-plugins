---
name: code-review
description: 仅当用户明确要求对 MR/PR、commit、分支或代码差异执行通用代码审查时使用。审查质量、正确性、可维护性、性能和最佳实践，并返回结构化问题。不得用于仅安全审计、普通编码、功能实现或修复请求，也不得在用户明确要求 Bits Code Guard 时使用。
platforms:
  - trae
  - claude_code
  - cursor
...
This skill follows the **Agent Plugins 1.0** standard and works across multiple AI coding platforms. Tool references below use **generic names**; map them to your platform:
...
> **Platform Detection**: When loaded, the AI agent should auto-detect its platform and use the corresponding tool names. If unsure, the agent should fall back to using the most common tool names available.
...
- If the user does not specify a scope, use the platform's interactive feedback tool to ask the user for clarification before proceeding.
...
- You **MUST** call tools (such as `code_search`, `file_read`) to fetch all relevant source and design evidence before commenting. See the Cross-Platform Compatibility table above for platform-specific tool names.
...
- **Iterative Fix Loop**:
  ...
  - After completing the fixes, if there are remaining unfixed issues, present them again and use the platform's interactive feedback tool to ask the user for the next action.
...
1. **Prefer Interactive Feedback Tools**: If the platform provides interactive feedback tools, **always use them** to collect user feedback. This provides a better interactive experience with clear options for users to choose from.

2. **Fallback to Text-based Inquiry**: If interactive feedback tools are **not available**, use clear text-based questions with numbered options for the user to respond to.
...
**Interactive tool inquiry example for fix selection (Preferred):**
```
Use the platform's interactive feedback tool with:
...
**Interactive tool inquiry example for remaining issues (after partial fix):**
```
Use the platform's interactive feedback tool with:
...
**Interactive tool inquiry example for scope clarification (Preferred):**
```
Use the platform's interactive feedback tool with:

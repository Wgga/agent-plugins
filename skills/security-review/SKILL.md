...
Prefer the platform's interactive feedback tool with these four options:
...
If interactive feedback tools are unavailable, ask the same four options as numbered plain text.
...
4. Re-prompt the user via the platform's interactive feedback tool for an explicit scope.

If a single auxiliary tool (e.g. `code_search`) is intermittently unavailable, continue with the rest and explicitly mark conclusions as evidence-bounded.
...
3. Use `code_search` to locate (a) input entry points / trust boundaries, (b) existing sanitizer / validator / authZ helpers in the project.
4. Use `file_read` to inspect the **full** body of touched files plus the relevant call-chain neighbors.
